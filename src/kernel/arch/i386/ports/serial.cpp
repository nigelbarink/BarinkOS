#include "serial.h"


void SerialHub::print(const char* message){
    uint8_t length  = strlen(message);

    for ( uint8_t i = 0; i < length; i++ ){
        write_character(message[i]);
    }
}

void SerialHub::println(const char* message){
    print(message+'\n');
}

SerialHub::SerialHub(){
    auto terminal = TextMode();

    /*Initialize 4 serial communication ports */
    int result = init(COM1);
    if (result != 0 )
        terminal.println ("COM1 failed to initialize!");

    result = init(COM2);
    if (result != 0 )
        terminal.println ("COM2 failed to initialize!");

    result = init(COM3);
    if (result != 0 )
        terminal.println ("COM3 failed to initialize!");

    result = init(COM4);
    if (result != 0 )
        terminal.println ("COM4 failed to initialize!");

    // set the default COM port to COM1
    PORT = COM1;

    test();
    
}

SerialHub::~SerialHub(){
    // TODO: Do some deinit stuff?!!?
}

int SerialHub::init(int PORT) {
   outb(PORT + 1, 0x00);    // Disable all interrupts
   outb(PORT + 3, 0x80);    // Enable DLAB (set baud rate divisor)
   outb(PORT + 0, 0x03);    // Set divisor to 3 (lo byte) 38400 baud
   outb(PORT + 1, 0x00);    //                  (hi byte)
   outb(PORT + 3, 0x03);    // 8 bits, no parity, one stop bit
   outb(PORT + 2, 0xC7);    // Enable FIFO, clear them, with 14-byte threshold
   outb(PORT + 4, 0x0B);    // IRQs enabled, RTS/DSR set
   outb(PORT + 4, 0x1E);    // Set in loopback mode, test the serial chip
   outb(PORT + 0, 0xAE);    // Test serial chip (send byte 0xAE and check if serial returns same byte)
 
   // Check if serial is faulty (i.e: not same byte as sent)
   if(inb(PORT + 0) != 0xAE) {
      return 1;
   }
 
   // If serial is not faulty set it in normal operation mode
   // (not-loopback with IRQs enabled and OUT#1 and OUT#2 bits enabled)
   outb(PORT + 4, 0x0F);
   return 0;
}

int SerialHub::is_transmit_empty() {
   return inb(PORT + 5) & 0x20;
}
 
void SerialHub::write_character(char a) {
   while (is_transmit_empty() == 0);
 
   outb(PORT,a);
}

int SerialHub::received() {
   return inb(PORT + 5) & 1;
}
 
char SerialHub::read() {
   while (received() == 0);
 
   return inb(PORT);
}

void SerialHub::test(){
        auto terminal = TextMode();
        /** Serial test **/
        terminal.println("Writing to COM1 serial port.");
        terminal.println("TEST 123...\n");

        println("TEST 123...");
}
